...
urlpatterns = [
    ...
    # Приватный endpoint для владельца (требуется аутентификация)
    path('manage/<str:username>/<str:unique_id>/', WebsiteRetrieveUpdateDestroyView.as_view(), name='website-detail'),
    # Публичный endpoint для просмотра сайта (без аутентификации)
